      ...
      {totalRevenueData?.map((item, idx) => (
        <div key={idx} className="dashboard-card flex-col items-stretch text-center flex-grow ">
          ...
          <h2 className="flex justify-center items-baseline flex-wrap">
            <span className={`text-3xl xl:text-4xl font-bold mr-2 ${item.textColor}`}>
              {formatValue(item.value, item.format)}
            </span>
            <span
              className={`text-xl font-semibold shrink-0 ${item.trend.direction === "up"
                ? "text-green-600" : (item.trend.direction === "down"
